...
class HomeEmployee:
    ...
    def create_menu(self, event):
        if self.menu is not None:
            ...
            vol_menu = tk.Menu(self.menu, tearoff=0)
            client_menu = tk.Menu(self.menu, tearoff=0)
            vente_menu = tk.Menu(self.menu, tearoff=0)

            vol_menu.add_command(label="MAJ vol dispo", command=self.save)
            vol_menu.add_command(label="Diverse offre reduc", command=self.save)
            client_menu.add_command(label="Gestion dossier client", command=self.window_file_management)
            client_menu.add_command(label="Historique resa", command=self.save)
            client_menu.add_command(label="nb_billet_acheter3mois", command=self.save)
            vente_menu.add_command(label="Analyse des ventes", command=self.save)
            vente_menu.add_command(label="Montant vente vol particulier", command=self.save)
            self.menu.add_cascade(label="Vol", menu=vol_menu)
            self.menu.add_cascade(label="Client", menu=client_menu)
            self.menu.add_cascade(label="Ventes", menu=vente_menu)
            self.menu.post(x, y)

    def window_file_management(self):
        client_window = tk.Toplevel(self.root)
        client_window.title("Gestion dossier client")
